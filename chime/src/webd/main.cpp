...
constexpr const char* kNetworkRestartCommand =
...
int main(int argc, char* argv[]) {
  ...
  const std::string chime_config_path =
      EnvOrDefault("CHIME_WEBD_CHIME_CONFIG", kChimeConfigPath);
  const std::string wpa_supplicant_path =
      EnvOrDefault("CHIME_WEBD_WPA_SUPPLICANT", kWpaSupplicantPath);
  ...
  const std::string host_label =
      EnvOrDefault("CHIME_WEBD_HOST_LABEL", kHostLabel);
  const std::string wifi_interface_override =
      vc::util::GetEnv("CHIME_WEBD_WIFI_INTERFACE");
  const std::string wifi_interface =
      wifi_interface_override.empty()
          ? ReadWifiInterfaceOrDefault(chime_config_path)
          : wifi_interface_override;
  const std::string network_restart_command = EnvOrDefault(
      "CHIME_WEBD_NETWORK_RESTART_CMD", kNetworkRestartCommand);
  const std::string chime_restart_command =
      EnvOrDefault("CHIME_WEBD_CHIME_RESTART_CMD", kChimeRestartCommand);
  const bool mdns_enabled = EnvBoolOrDefault("CHIME_WEBD_MDNS_ENABLED", true);

  chime::webd::ConfigStore config_store(logger, chime_config_path,
                                        wpa_supplicant_path);
  chime::webd::WifiScanner wifi_scanner(logger, wifi_interface);
  chime::webd::ApplyManager apply_manager(logger, network_restart_command,
