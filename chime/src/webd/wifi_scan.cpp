...
CommandResult RunCommand(const std::vector<std::string>& args, int timeout_ms,
  ...
  if (pid == 0) {
    ...
    if (args[0].find('/') != std::string::npos) {
      ...
      constexpr const char* kLookupDirs[] = {"/usr/sbin", "/sbin", "/usr/bin",
                                             "/bin"};
    ...
    if (poll_rc > 0 && (pfd.revents & POLLIN)) {
      ...
      if (bytes > 0) {
        ...
        const std::size_t to_copy =
            std::min<std::size_t>(remaining, static_cast<std::size_t>(bytes));
        if (to_copy > 0) {
...
std::string DescribeCommandFailure(const std::string& command,
...
#if defined(__APPLE__)
std::string JoinFieldsFrom(const std::vector<std::string>& fields,
...
WifiScanResult ScanWithAirport() {
  ...
  const CommandResult scan_output =
      RunCommand({kAirportPath, "-s"}, kScanTimeoutMs, kMaxCommandOutputBytes);
  if (!scan_output.success) {
    result.error = "airport scan command failed";
    return result;
  }

  static const std::regex bssid_regex(
  ...
  for (const auto& raw_line : SplitLines(scan_output.output)) {
    ...
    const std::string remainder = vc::config::trim(
...
WifiScanResult WifiScanner::ScanWithWpaCli() const {
  ...
  const CommandResult trigger =
      RunCommand({"wpa_cli", "-i", interface_name_, "scan"}, kScanTimeoutMs,
                 kMaxCommandOutputBytes);
  ...
  for (int attempt = 0; attempt < kScanResultsAttempts; ++attempt) {
    ...
    const CommandResult scan_results =
        RunCommand({"wpa_cli", "-i", interface_name_, "scan_results"},
                   kScanTimeoutMs, kMaxCommandOutputBytes);
    if (!scan_results.success) {
      last_scan_results_error =
          DescribeCommandFailure("wpa_cli scan_results", scan_results);
      continue;
    }
...
WifiScanResult WifiScanner::ScanWithIw() const {
  ...
  const CommandResult iw_output =
      RunCommand({"iw", "dev", interface_name_, "scan"}, kScanTimeoutMs,
                 kMaxCommandOutputBytes);
