...
bool ScalePcmWavInMemory(std::vector<uint8_t>* wav_bytes, int effective_volume,
                         std::string* error) {
  ...
  while (cursor + 8 <= wav_bytes->size()) {
    ...
    const std::string chunk_id(
        reinterpret_cast<const char*>(wav_bytes->data() + chunk_header), 4);
    if (chunk_id == "fmt ") {
      if (chunk_size < 16) {
        if (error != nullptr) {
    ...
    cursor = chunk_end + (chunk_size % 2 == 1 ? 1 : 0);
  }
  ...
  if (bits_per_sample == 16) {
    for (std::size_t i = 0; i + 1 < data_size; i += 2) {
      ...
      const int16_t sample = static_cast<int16_t>(
          static_cast<uint16_t>((*wav_bytes)[sample_offset]) |
      ...
      WriteLe16(wav_bytes, sample_offset,
  ...
  if (bits_per_sample == 8) {
    for (std::size_t i = 0; i < data_size; ++i) {
      ...
      const int scaled =
          static_cast<int>(std::lround(static_cast<double>(centered) * gain));
...
bool CreateSoftwareScaledWav(const std::string& source_path, int effective_volume,
  ...
  if (!WriteFileBytes(temp_path, wav_bytes)) {
...
MixerSetResult TrySetVolumeWithAmixer(int effective_volume) {
  for (const char* control_name : kMixerControlCandidates) {
    const std::string set_volume_cmd =
        "amixer -q sset \"" +
        vc::util::EscapeShellDoubleQuotes(control_name) + "\" \"" +
...
void AplayAudioPlayer::Play(const std::string& path, int volume_percent) {
  ...
  std::thread([this, path, effective_volume]() {
    ...
    if (!mixer_result.success) {
      logger_.Warn(
          "audio",
          "failed to set volume via amixer using known controls; ring volume "
          "setting may have no audible effect (tried: " +
              MixerCandidatesForLog() + ")");

      if (effective_volume < 100) {
        std::string scale_error;
        if (CreateSoftwareScaledWav(path, effective_volume, &temporary_scaled_path,
                                    &scale_error)) {
          playback_path = temporary_scaled_path;
          logger_.Info("audio", "using software-scaled wav fallback at " +
                                    std::to_string(effective_volume) + "%");
        } else {
          logger_.Warn("audio", "software volume fallback unavailable: " +
                                    scale_error);
        }
      }
    } else {
      logger_.Info("audio", "applied mixer control '" + mixer_result.control_name +
                                "' to " + std::to_string(effective_volume) + "%");
    }

    const std::string cmd =
        "aplay -q \"" + vc::util::EscapeShellDoubleQuotes(playback_path) +
        "\" 2>/dev/null";
